...
#conversion of Area from meter Square to Acre

acre=`awk 'BEGIN{printf("%0.10f", '$area' * 0.000247105)}'`
		echo "$area meter square is $acre Acre"

read -p "Enter number of  plots: " plots;
total_Area=`awk 'BEGIN{printf("%0.10f", '$acre' * 25)}'`
echo $total_Area
